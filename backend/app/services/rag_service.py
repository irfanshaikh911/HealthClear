"""
RAG Service — free-form conversational AI for healthcare cost estimation.
Uses LangChain + Groq to extract fields from natural language, asks follow-ups,
and runs the cost engine when all fields are collected.
"""
import json
import os
import re
...
def build_knowledge_context(client: Client) -> str:
    """Fetch all hospitals, procedures, risk conditions and format as text block."""
    lines = []
...
def build_patient_context(client: Client, patient_id: int) -> str:
    """Fetch a patient's profile + active medical history as text."""
    patient_res = (
...
def process_rag_turn(
    ...
    patient_ctx_block = ("Patient profile from records:\n" + patient_context) if patient_context else ""
