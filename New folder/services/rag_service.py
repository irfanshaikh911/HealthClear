...
def _extract_fields(
    ...
    try:
        ...
        # Merge: only add truly new keys (don't overwrite what we have)
        for k, v in extracted.items():
            if k not in collected and v is not None and v != "" and v != []:
                collected[k] = v
    except Exception:
...
Also return 3-6 suggested option buttons that would answer your question.
Return JSON:
{
...
- Keep answers concise (3-5 sentences for simple questions, a short paragraph for complex ones).
...
def _generate_completion_message(collected: dict, knowledge_context: str, patient_context: str) -> str:
    ...
    system = SystemMessage(content=(
        "You are HealthClear. Write a brief, warm 1-2 sentence message telling the patient "
        "you have everything you need and are now preparing their personalized recommendation. "
        "Mention the specific condition/symptom and city. "
        f"Action being taken: {action}. Be enthusiastic but professional."
    ))
...
def build_doctor_recommendation(collected: dict, client) -> dict:
    ...
    # Fallback: try GP if specialist not found
    if not doctors:
        doctors = (
            client.table("doctor")
            .select("*")
