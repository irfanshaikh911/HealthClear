"""
Patient Service
Fetches patient profile + medical history from Supabase,
maps conditions to known risk factors via Groq, and generates
a personalized opening summary.
"""
import json
import os
...
def fetch_patient_context(client: Client, patient_id: int) -> Optional[dict]:
    """
    Returns a dict with patient profile + active medical history,
    or None if patient not found.
    """
    patient_res = (
...
def map_conditions_to_risk(conditions: list[str]) -> dict:
    """
    Uses Groq to map arbitrary condition names to our known risk factors.
...
def prefill_from_patient(patient_ctx: dict) -> tuple[dict, list[str]]:
    """
    Extracts known ChatRequest fields from patient data.
    ...
    # Check smoking from medications/notes (simple keyword scan)
    all_notes = " ".join(
